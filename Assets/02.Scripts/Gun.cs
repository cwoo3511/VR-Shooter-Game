...
public class Gun : MonoBehaviour
{
    ...
    private float m_LastFireTime;   // 총을 마지막을 발사한 시점
    private int m_CurrentAmmo = 13;  // 탄창에 남은 현재 탄약 갯수

    void Start()
    {
        m_CurrentState = State.Ready;   // 탄약이 빈 상태로 지정
    ...
    public void Shot()
    {
        ...
        if (Physics.Raycast(m_FireTransform.position, m_FireTransform.forward
            ,out hit, m_FireDistance))
        {
            // 상대방이 IDamageAble 로서 가져와진따면,
            // 상대방의 OnDamage함수를 실행시켜서 데미지를 쥐어준다
